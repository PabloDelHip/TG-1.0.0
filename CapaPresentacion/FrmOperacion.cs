            ...
            else if (MessageBox.Show("Cerrar venta?", "Continuar", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                if (Login.Pago)
                {
                    ...
                    for (int i = 0; i < lista_datos_venta.Count; i++)
                    {
                        ...
                        if(cortesiaVisita)
                        {
                            cls_mov_visitas.m_tipo = lista_datos_venta[i].Tipo;
                            cls_mov_visitas.m_folioVenta = FolioVenta;
                            string respuesta = cls_mov_visitas.movVisitasInsert();
                            idSocio = respuesta;
                        ...
                        DS.Tabla.Rows.Add(Login.nombre, idSocio, nombreVenta, lista_datos_venta[i].Item, "$" + lista_datos_venta[i].Monto, cls_generales.enletras(SubtotalAPagar.ToString()), FolioVenta.ToString(), "$" + SubtotalAPagar);
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            else if(btnModificarSocio.Text.Equals("Guardar Modificacion"))
            {
                ...
                else if (ChkForm_InsOGuardar())
                {
                    ...
                    cls_socios.m_Observacion = "Observaciones";
                    cls_socios.m_User_modif = Login.nombre;
                    System.IO.MemoryStream ms = new System.IO.MemoryStream();
                    // Se guarda la imagen en el buffer
                    pbFotoUser.Image.Save(ms, System.Drawing.Imaging.ImageFormat.Jpeg);
